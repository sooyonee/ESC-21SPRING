# 기존의 데이터 structure 코드는 write.csv를 통해서 저장하고 read.csv를 통해서 불러왔다.
Y.school.mathscore <- read.csv('week5_HWdata.csv')
...
for(j in 1:J) {
  ...
  n[j]<-length(Y[[j]])
  s2[j]<-var(Y[[j]])
  YM<-rbind( YM, cbind( rep(j,n[j]), Y[[j]] )) #list인 Y를 하나의 행렬형태 YM로 만들기
}
...
for(s in 1:S){
  ...
  for(j in 1:m) 
  {
    ...
    etheta<-vtheta*(ybar[j]*n[j]/sigma2+mu/tau2) # 채워 넣었음!#############################
  ...
  ss<- eta0*t20 + sum( (theta-mu)^2 ) # 채워 넣었음!#####################################
